...
export type Product = {
  ...
  tag?: Tag;
  category: Category;
  // Only meaningful when category is "Clothing" -- one of
  // CLOTHING_SUBCATEGORIES[vertical] below. Undefined for every other
  // category, and undefined for Clothing products that haven't been given
  // one (optional, not required).
  subcategory?: string;
  vertical: Vertical;
  emoji: string;
  color: string;
...
// Subcategories are Clothing-only, distinct per vertical (e.g. Kids has no
// "Ethnic Wear", Women has no "Sleepwear"). Single source of truth for the
// admin panel's subcategory dropdown, the Excel import's validation, and
// anywhere else that needs the allowed list.
export const CLOTHING_SUBCATEGORIES: Record<Vertical, string[]> = {
  kids: ["Onesies", "T-shirts", "Bottoms", "Sleepwear", "Outerwear"],
  women: ["Tops", "Dresses", "Jeans", "Ethnic Wear", "Skirts & Co-ords"],
  men: ["T-shirts", "Shirts", "Jeans", "Trousers", "Jackets", "Shorts", "Sweatshirts & Hoodies", "Ethnic Wear"],
};

export function getClothingSubcategories(vertical: Vertical): string[] {
  return CLOTHING_SUBCATEGORIES[vertical];
}

export function isValidClothingSubcategory(vertical: Vertical, subcategory: string): boolean {
  return CLOTHING_SUBCATEGORIES[vertical].includes(subcategory);
}

// One-time seed data — migrated into the SQLite `products` table on first run (see lib/products.ts).
// NOTE: the ids below intentionally start at 9 -- ids 1-8 were Diapers/Feeding
// products, categories removed entirely from the catalog (2026-08-10).
